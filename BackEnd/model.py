...
class QwenScorer:
    ...
    def generate_json(self, essay: str, hsk_level: int) -> str:
        ...
        # --- ULTIMATE PROMPT TUNING FIX ---
        # Force the LLM to analyze the specific input essay with extreme precision.
        prompt = f"""
...
INPUT ESSAY (HSK {hsk_level}, Analyze this text ONLY):
{essay}
...
CRITICAL INSTRUCTIONS FOR ERROR DETECTION:
...
- For EACH identified error of the three types listed above:
   ...
   b. DETERMINE THE ZERO-BASED starting character index (inclusive) and the ending character index (exclusive) of this substring within the 'INPUT ESSAY'. PYTHON SLICING `essay[start:end]` MUST YIELD THE `incorrect_fragment`. This is `error_position`.
...
EXAMPLE OF A GOOD ERROR DETECTION (DO NOT COPY THIS EXACTLY, JUST FOLLOW THE FORMAT):
If the INPUT ESSAY was "我吃饭在餐厅", the error would be:
{{
  "error_type": "word_order",
  "error_position": [1, 5], // Because essay[1:5] is "吃饭在"
